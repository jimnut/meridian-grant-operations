...
- **Stripe** (`server/lib/stripe.ts`, `server/routes/billing.ts`): hosted Checkout in subscription mode (branded as GrantConsole per session, with the founding-customer coupon from `shared/plans.ts` `FOUNDING_OFFER` applied automatically while places remain; the coupon is found by metadata or created on first use and limited to GrantConsole's products), the customer portal (opened with a GrantConsole-branded portal configuration that the server finds by metadata or creates on first use, so a shared Stripe account still shows GrantConsole's headline, legal links and plan switches), and a signature-verified webhook (`POST /api/billing/webhook`, raw body, idempotent per event id) that keeps `organizations.plan/subscription_status/plan_valid_until` in step. Everything is gated on env; without keys the billing page offers an email/invoice path.
